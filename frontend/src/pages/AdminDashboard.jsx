...
const StatCard = ({ title, value, icon: Icon, trend, color, loading }) => (
    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 flex flex-col hover:shadow-md transition-shadow cursor-default">
...
const AdminDashboard = () => {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            <div>
                <h1 className="text-2xl font-bold text-aura-dark dark:text-white">Admin Intelligence Console</h1>
                <p className="text-gray-400 text-sm font-medium">Global oversight of all complaint vectors</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 flex flex-col h-[400px]">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="font-bold text-aura-dark dark:text-white">Global Complaint Volume</h3>
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 flex flex-col h-[400px]">
                    <h3 className="font-bold text-aura-dark dark:text-white mb-6">Classification Breakdown</h3>
            ...
            <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="font-bold text-aura-dark dark:text-white text-lg">Recent Customer Submissions</h3>
                    <button className="text-aura-light text-sm font-bold flex items-center hover:underline px-4 py-2 bg-aura-light/5 rounded-xl transition-all">
                        View Intelligence Center <ArrowRight className="w-4 h-4 ml-2" />
                    </button>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead>
                            <tr className="text-[10px] uppercase tracking-widest text-gray-400 font-bold border-b border-gray-50 dark:border-slate-700">
                                <th className="px-4 py-4">Customer</th>
                                <th className="px-4 py-4">Complaint Vector</th>
                                <th className="px-4 py-4 text-center">AI Category</th>
                                <th className="px-4 py-4 text-right">Status</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-50 dark:divide-slate-700/50">
                            {recentComplaints.map((c) => (
                                <tr key={c._id} className="group hover:bg-gray-50/50 dark:hover:bg-slate-700/30 transition-all">
                                    <td className="px-4 py-5">
                                        <div className="flex items-center space-x-3">
                                            <div className="w-8 h-8 rounded-full bg-aura-light/10 flex items-center justify-center text-aura-light font-bold text-xs">
                                                {c.user?.name?.charAt(0) || 'U'}
                                            </div>
                                            <div>
                                                <p className="text-sm font-bold text-aura-dark dark:text-white">{c.user?.name || 'Anonymous'}</p>
                                                <p className="text-[10px] text-gray-400">{c.user?.email || 'No email'}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-4 py-5">
                                        <p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-1 max-w-xs">{c.text}</p>
                                    </td>
                                    <td className="px-4 py-5 text-center">
                                        <span className="text-[10px] font-bold px-2 py-1 bg-aura-light/10 text-aura-light rounded-lg">
                                            {c.category}
                                        </span>
                                    </td>
                                    <td className="px-4 py-5 text-right">
                                        <div className="flex items-center justify-end space-x-2">
                                            <div className={`w-1.5 h-1.5 rounded-full ${c.status === 'resolved' ? 'bg-green-500' : 'bg-aura-orange animate-pulse'}`}></div>
                                            <span className="text-xs font-bold text-aura-dark dark:text-gray-300 capitalize">{c.status}</span>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                            {recentComplaints.length === 0 && (
                                <tr>
                                    <td colSpan="4" className="text-center py-12 text-gray-400 text-sm">No recent activity detected</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
